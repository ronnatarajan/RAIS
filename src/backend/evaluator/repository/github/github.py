...
class Github:
    def __init__(self, owner, repo, branch, depot: Depot):
        ...
        if (owner, repo, branch) in [
            (x["owner"].replace(".json", "").replace("../depot/repository/owner/", ""), x["repo"], x["branch"])
            for x in depot.mapping.values()
            if x
        ]:
    ...
    def metadata(self):
        return {
            "commit_history": [
                list(x)
                for x in zip(
                    self.metadata_object.root_metadata["commit_name"], self.metadata_object.root_metadata["commit_date"]
                )
            ],
            ...
            "metadata": {
                ...
                if key != "commit_name" or key != "commit_date"
            },
        }
    ...
    def save(self, path):
        ...
        return {
            ...
            "organization": os.path.join(path, "organization", f'{self.metadata["organization"]["name"]}.json')
    ...
    def extract_code_elements(self, file_path):
        if file_path.endswith(".py"):
            ...
            called_functions = [
                ...
                if func not in common_python_functions and "__" not in func and len(func.replace("__", "")) > 5
            ]
            ...
            for child in node["children"]:
                if child.get("type") == "file":
                    ...
                    (
                        ...
                        child_subfiles,
                        child_files_direct,
                        child_folders_direct,
                        child_file_types,
                    ) = self.enrich_tree_attributes(child)
